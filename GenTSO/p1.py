...
axs.errorbar(obs_wl, obs_depth*1e6, yerr=obs_error*1e6, fmt='o', ms=5, color='xkcd:blue', mfc=(1,1,1,0.85), label='simulated (noised up) transit spectrum')
...
# -------- Saving the data --------
np.savetxt(os.getcwd() + f'/GenTSO/Data/toi396b_transit_spectrum_isothermal_{instrument}_R250.txt', np.array([obs_wl, obs_depth*1e6, obs_error*1e6]).T, header='Wavelength [micron], Transit depth [ppm], Transit depth error [ppm]')